...
# from qlib import fname_to_code, code_to_fname
# import qlib
# print(qlib.__file__)
def fname_to_code(fname: str) -> str:
    """
    Convert a filename (without extension) to a standardized instrument code.

    Example:
        fname_to_code("sh600000") -> "SH600000"

    Parameters:
    ----------
    fname: str
        Filename or basename representing instrument, e.g., 'sh600000.csv' or 'sh600000'.

    Returns:
    -------
    str
        Uppercase instrument code without file extension.
    """
    # Remove extension if present
    code = fname.split('.')[-2] if '.' in fname else fname
    return code.upper()


def code_to_fname(code: str) -> str:
    """
    Convert an instrument code to a standardized filename (directory name) for Qlib features.

    Example:
        code_to_fname("SH600000") -> "sh600000"

    Parameters:
    ----------
    code: str
        Instrument code, e.g., 'SH600000'
